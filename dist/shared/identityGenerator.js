 ...
 */
export function generateCharacterName(interests = []) {
 ...
 */
export function selectAvatarStyle(interests = []) {
    const styles = [
        'shadow', 'cyber', 'fantasy', 'minimal', 'toon', 'mystic', 'space', 'samurai', 'retro', 'neon'
    ];
    const normalized = interests.map(i => i.toLowerCase());
    if (normalized.some(i => i.includes('anime') || i.includes('manhwa')))
        return 'samurai';
    if (normalized.some(i => i.includes('game') || i.includes('gaming')))
        return 'cyber';
    if (normalized.some(i => i.includes('music') || i.includes('k-pop')))
        return 'neon';
    if (normalized.some(i => i.includes('space') || i.includes('science')))
        return 'space';
    if (normalized.some(i => i.includes('art') || i.includes('photo')))
        return 'mystic';
    if (normalized.some(i => i.includes('memes') || i.includes('casual')))
        return 'toon';

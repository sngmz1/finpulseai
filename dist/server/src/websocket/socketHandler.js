...
export function setupSocketHandler(io, database = db, rooms = roomService, matchmaking = matchmakingService) {
    io.on('connection', (socket) => {
        ...
        // --- ROOM ACTIONS ---
        socket.on('room:create', async ({ type, maxUsers, interests }, callback) => {
            try {
                const currentUser = database.getUser(internalUserId);
                if (!currentUser) {
                    if (typeof callback === 'function')
                        callback({ success: false, error: 'User session not found' });
                    return;
                }
        ...
        socket.on('room:join', async ({ roomIdOrCode, inviteToken }, callback) => {
            try {
                const currentUser = database.getUser(internalUserId);
                if (!currentUser) {
        ...
        socket.on('voice:set_mute', (isMuted) => {
            const roomId = socketToRoom.get(socket.id);
            if (!roomId)
                return;
        ...
        socket.on('voice:set_state', (voiceState) => {
            const roomId = socketToRoom.get(socket.id);
            if (!roomId)
                return;
        ...
        socket.on('webrtc:send_offer', (data) => {
        ...
        socket.on('webrtc:send_answer', (data) => {
            const targetSockets = userToSockets.get(data.toUserId);
            if (targetSockets) {
                targetSockets.forEach(targetId => {
        ...
        socket.on('webrtc:send_ice_candidate', (data) => {
            const targetSockets = userToSockets.get(data.toUserId);
            if (targetSockets) {
                targetSockets.forEach(targetId => {
        ...
        socket.on('chat:delete_message', (messageId, callback) => {
            const roomId = socketToRoom.get(socket.id);
            if (!roomId) {
        ...
        // --- MATCHMAKING ACTIONS ---
        socket.on('match:start', async ({ interests, language }, callback) => {
            try {
                ...
                if (typeof callback === 'function')
                    callback({ success: false, error: err.message || 'Failed to enter matching queue' });
            }
        });
        ...
        // --- SAFETY ACTIONS ---
        socket.on('safety:report', ({ reportedPublicId, reason, details, messageId }, callback) => {
        ...
        socket.on('safety:block', ({ targetPublicId }, callback) => {

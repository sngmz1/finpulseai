...
import { AnonymousUser, Room, RoomMember } from '../../../shared/types.js';
...
export class MatchmakingService {
  ...
  private queue: Map<string, QueuedUser> = new Map(); // userId -> QueuedUser
  private cleanupInterval: NodeJS.Timeout | null = null;
  public static readonly QUEUE_TIMEOUT_MS = 45000; // 45 seconds

  constructor(database: Database = db, rooms: RoomService = roomService) {
  ...
  public addToQueue(user: AnonymousUser, interests: string[], socketId: string, language: string = 'en'): QueuedUser {
    // Check if banned
    if (user.isBanned) {
  ...
  public getQueuedUser(userId: string): QueuedUser | undefined {
   ...
   */
  public calculateMatchScore(interestsA: string[], interestsB: string[]): number {
    if (interestsA.length === 0 && interestsB.length === 0) return 0.5; // generic match
   ...
   * STRICT CONSTRAINT: Random room NEVER exceeds 3 users.
   */
  public async findMatch(userId: string): Promise<{ 
    matched: boolean; 
    room?: Room; 
    ...
    for (const room of this.database.rooms.values()) {
      if (
        ...
        const hasBlockConflict = room.members.some((member: RoomMember) => 
          this.database.isBlocked(candidate.user.publicId, member.publicId)
        );
        if (!hasBlockConflict) {
          openRooms.push(room);
        }
    ...
    let highestScore = -1;
    let bestMatchedInterests: string[] = [];
    ...
    for (const room of openRooms) {
      ...
      if (score > highestScore) {
        ...
        bestMatchedInterests = room.interests.filter((i: string) => candidateSet.has(i));
      }
    }
    ...
    if (bestRoom) {
      try {
        const existingMemberIds = bestRoom.members.map((m: RoomMember) => m.userId);
    ...
    const queuedCandidates: QueuedUser[] = [];
   ...
   */
  public async findMatchesForQueue(): Promise<Array<{
    room: Room;
    matchedInterests: string[];
    matchedUserIds: string[];
  }>> {
